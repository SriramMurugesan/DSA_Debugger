...
export function LinkedListVisualizer({ structure }: { structure: any }) {
  const nodes = structure.data || [];
  const references = structure.references || {};
  
  // Find which variables point to a specific node ID
  const getPointersForNode = (id: string) => {
    return Object.keys(references).filter(key => references[key] === id);
  };

  const nameLabel = Object.keys(references).join(', ') || 'Linked List';
